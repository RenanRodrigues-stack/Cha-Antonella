...
document.getElementById("form-confirmacao").addEventListener("submit", async (event) => {
  ...
  const erroItens = document.getElementById("erro-itens");

  if (!nome) return;

  const checkboxesMarcados = document.querySelectorAll('#lista-itens input[type="checkbox"]:checked');
  const totalCheckboxes = document.querySelectorAll('#lista-itens input[type="checkbox"]').length;

  if (checkboxesMarcados.length === 0 && totalCheckboxes > 0) {
    erroItens.style.display = "block";
    erroItens.scrollIntoView({ behavior: "smooth", block: "center" });
    return;
  }
